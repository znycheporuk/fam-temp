...
export const getResetPasswordTemplate = (lang: TLang, params: IGetResetPasswordTemplateParams) => {
  ...
  ${lang === "en" ? `
    ...
    <p>To reset password follow the <a href="${process.env.ORIGIN}/${lang}/reset-password?token=${params.token}&userId=${params.userId}">link</a>, it will expire in 15 minutes.</p>
    <p>If you didn't request it, just ignore this email, and make sure no one has access to it.</p>
  ` : `
    <h3>Привіт ${params.firstName}!</h3>
    <p>Щоб відновити пароль, перейдіть за <a href="${process.env.ORIGIN}/${lang}/reset-password?token=${params.token}&userId=${params.userId}">посиланням</a>, воно дійсне 15 хвилин.</p>
